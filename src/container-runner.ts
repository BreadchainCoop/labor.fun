...
export function writeEventsSnapshot(
  ...
  const eventsFile = path.join(groupIpcDir, 'current_events.json');
  fs.writeFileSync(eventsFile, JSON.stringify(events, null, 2));
}

export interface TourSnapshotSlot {
  id: string;
...
export interface TourSnapshotPotential {
  event_id: string;
  title: string;
  description: string | null;
  start_time: string;
...
export interface TourSnapshot {
  upcoming: TourSnapshotSlot[];
  past: TourSnapshotSlot[];
  potential: TourSnapshotPotential[];
}

export function writeToursSnapshot(
  groupFolder: string,
  ...
  const toursFile = path.join(groupIpcDir, 'current_tours.json');
  fs.writeFileSync(toursFile, JSON.stringify(snapshot, null, 2));
}
