...
function createSchema(database: Database.Database): void {
  ...
  // --- Residency (rooms, room_occupancy, residency_requests) ---
  // Single schema owner for the residency plugin's data layer. Upstream
  // (lil_salem) had a dual-DDL split-brain: the orchestrator's rooms table
  // (room_number UNIQUE, no map columns) disagreed with the control panel's
  // (no UNIQUE, map columns bolted on via ALTER). This is the merged fix:
  // orchestrator constraints PLUS the building-map columns first-class.
  // building_id/floor_id/map_room_id link a room to a physical room in the
  // profile's buildings.json (see container/catalog-plugins/residency/).
  database.exec(`
...
/**
 * @internal - for tests only. Creates a fresh database (in-memory by default;
 * pass a file path when another process/connection must open the same DB —
 * e.g. the kb-ui plugin router tests, which exercise a separate connection
 * against a schema created by this single owner).
 */
export function _initTestDatabase(dbPath: string = ':memory:'): void {
  db = new Database(dbPath);
  createSchema(db);
}
...
// --- Residency accessors (rooms / room_occupancy / residency_requests) ---
// Ported from lil_salem with the dual-DDL split-brain fixed: this module is
// the ONLY schema owner, and the map columns (building_id/floor_id/map_room_id)
// are first-class on create/update — agent-created rooms carry them too.
...
export interface RoomOccupancy {
  ...
  /** Joined from app_users when reading. */
...
/**
 * Today's civil date (`YYYY-MM-DD`) in the org's configured TIMEZONE.
 * Occupancy windows are calendar dates in the venue's zone, NOT the server's
 * UTC clock — a stay ending "today" is still current at 11pm venue time even
 * when UTC has rolled over. Mirrors lil_salem's venue-time semantics
 * (venueToday) using the framework's profile-driven TIMEZONE.
 */
export function residencyToday(now: Date = new Date()): string {
...
export function getAllRooms(): Room[] {
  ...
  for (const room of rooms) {
    room.occupancy = occStmt.all(room.id) as RoomOccupancy[];
  }
  return rooms;
}
...
export function updateRoom(
  ...
  if (updates.room_name !== undefined) {
    ...
    values.push(updates.room_name || null);
  }
  if (updates.capacity !== undefined) {
    fields.push('capacity = ?');
    values.push(updates.capacity || 1);
  }
  ...
  if (updates.building_id !== undefined) {
    ...
    values.push(updates.building_id || null);
  }
  if (updates.floor_id !== undefined) {
    fields.push('floor_id = ?');
    values.push(updates.floor_id || null);
  }
  if (updates.map_room_id !== undefined) {
    fields.push('map_room_id = ?');
    values.push(updates.map_room_id || null);
  }
  if (fields.length > 0) {
    values.push(id);
...
/** Occupants whose stay covers today (venue-local civil day, see residencyToday). */
export function getRoomOccupantCount(roomId: string): number {
  ...
  const row = db
    ...
    .get(roomId, today, today) as { cnt: number } | undefined;
...
export function updateOccupancy(
  ...
  if (fields.length > 0) {
    values.push(id);
    db.prepare(
      `UPDATE room_occupancy SET ${fields.join(', ')} WHERE id = ?`,
    ).run(...values);
  }
  return db
    .prepare(
    ...
    .get(id) as RoomOccupancy | undefined;
}
...
// --- Residency request accessors (application intake) ---
