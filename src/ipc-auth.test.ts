...
// --- residency request workflow ---

describe('residency request IPC handlers', () => {
  // Track messages sent by deps.sendMessage so we can assert notifications
  ...
  it('submit_residency_request creates pending row and notifies main group', async () => {
    await processTaskIpc(
      {
    ...
    expect(pending[0].requester_name).toBe('Alex Applicant');
    expect(pending[0].chat_jid).toBe('other@g.us');
    expect(pending[0].source_group).toBe('other-group');
    expect(pending[0].status).toBe('pending');

    expect(sent).toHaveLength(1);
    expect(sent[0].jid).toBe('main@g.us');
    expect(sent[0].text).toContain('New residency request');
    expect(sent[0].text).toContain('Alex Applicant');
    expect(sent[0].text).toContain(pending[0].id);
  });

  it('submit_residency_request ignores untrusted data.chatJid and derives from sourceGroup', async () => {
    await processTaskIpc(
      {
        type: 'submit_residency_request',
        requester_name: 'Spoof Target',
        request_type: 'guest',
        requested_start_date: '2026-06-01',
        // Attempted spoof: chat_jid passed in payload should be ignored
        chatJid: 'third@g.us',
  ...
  it('submit_residency_request aborts when sourceGroup is unknown', async () => {
    await processTaskIpc(
      {
        type: 'submit_residency_request',
        requester_name: 'Orphan',
        request_type: 'resident',
        requested_start_date: '2026-06-01',
      },
      'not-registered',
    ...
    expect(listResidencyRequestsByStatus()).toHaveLength(0);
    expect(sent).toHaveLength(0);
  });

  it('submit_residency_request rejects invalid request_type', async () => {
    await processTaskIpc(
      {
        type: 'submit_residency_request',
        requester_name: 'Bad Type',
        request_type: 'investor',
        requested_start_date: '2026-06-01',
      },
      'other-group',
      false,
  ...
  it('submit_residency_request rejects end_date before start_date', async () => {
    await processTaskIpc(
      {
        type: 'submit_residency_request',
        requester_name: 'Backwards Dates',
        request_type: 'guest',
  ...
  it('review_residency_request from non-main is rejected', async () => {
    const req = createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Pending Person',
      request_type: 'resident',
      requested_start_date: '2026-06-01',
    });
    ...
    const after = getResidencyRequest(req.id);
    expect(after!.status).toBe('pending');
    expect(sent).toHaveLength(0);
  });

  it('review_residency_request approve transitions pending→approved and notifies origin', async () => {
    const req = createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Approve Me',
      request_type: 'resident',
      requested_start_date: '2026-06-01',
    });
    ...
    expect(after!.resolved_by).toBe('whatsapp_main');
    expect(after!.resolution_notes).toBe('welcome');
  ...
  it('review_residency_request reject transitions pending→rejected', async () => {
    const req = createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Reject Me',
      request_type: 'guest',
      requested_start_date: '2026-06-01',
    });
  ...
  it('review_residency_request refuses to re-resolve an already-resolved request', async () => {
    const req = createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Already Done',
      request_type: 'resident',
      requested_start_date: '2026-06-01',
    });
    ...
    // Second review attempt should be a no-op
    sent = [];
    await processTaskIpc(
      {
        type: 'review_residency_request',
  ...
  it('list_residency_requests from non-main is rejected (no notification sent)', async () => {
    createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
  ...
  it('list_residency_requests from main posts a listing and respects status filter', async () => {
    const a = createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Pending A',
      request_type: 'resident',
      requested_start_date: '2026-06-01',
    });
    createResidencyRequest({
      chat_jid: 'other@g.us',
      source_group: 'other-group',
      requester_name: 'Pending B',
      request_type: 'guest',
      requested_start_date: '2026-07-01',
    });
    // Approve the first so there's one approved + one pending
    await processTaskIpc(
      {
        type: 'review_residency_request',
        request_id: a.id,
    ...
    await processTaskIpc(
      { type: 'list_residency_requests', status: 'pending' },
      'whatsapp_main',
      true,
      deps,
    );
    ...
    expect(sent).toHaveLength(1);
    expect(sent[0].jid).toBe('main@g.us');
    expect(sent[0].text).toContain('Pending B');
    expect(sent[0].text).not.toContain('Pending A');
    expect(sent[0].text).toContain('status=pending');
  });
});
