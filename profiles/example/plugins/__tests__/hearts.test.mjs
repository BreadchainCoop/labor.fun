...
it('tick integration', async () => {
  ...
  fs.writeFileSync(path.join(chDir, 'c2.json'), JSON.stringify({
    status: 'new', challenger: 'bob', challengee: 'erin', value: 1,
  }));
  ...
  assert.equal(c2.loser, 'bob', 'failed challenge penalizes challenger');
  ledger = JSON.parse(fs.readFileSync(path.join(base, 'ledger.json')));
  assert.equal(heartsOf(ledger, 'bob'), 4);
  ...
  julyGrant('alice', 'bob', 1);
  julyGrant('carol', 'bob', 2);
  ...
  // floor(5/3) = 1 winner; bob has most influence.
  assert.equal(karmaEntries.length, 1, 'one karma winner for 5 residents');
  assert.equal(karmaEntries[0].resident, 'bob');
  ...
  // bob: deficiency 100 → -5
  assert.equal(choreEntries.find((e) => e.resident === 'bob').value, -5);
