...
describe('parameterized SQL registration', () => {
  ...
  it('registers a group with parameterized query', () => {
    ...
    expect(row.trigger_pattern).toBe('@Breadbrich Engels');
  ...
  it('prevents SQL injection in JID field', () => {
    ...
    db.prepare(
      `INSERT OR REPLACE INTO registered_groups
       (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger)
       VALUES (?, ?, ?, ?, ?, NULL, ?)`,
    ).run(maliciousJid, 'Evil', 'evil', '@Breadbrich Engels', '2024-01-01T00:00:00.000Z', 1);
...
describe('file templating', () => {
  it('replaces assistant name in CLAUDE.md content', () => {
    let content = '# Breadbrich Engels\n\nYou are Breadbrich Engels, a personal assistant.';

    content = content.replace(/^# Breadbrich Engels$/m, '# Nova');
    content = content.replace(/You are Breadbrich Engels/g, 'You are Nova');

    expect(content).toBe('# Nova\n\nYou are Nova, a personal assistant.');
  });

  it('handles names with special regex characters', () => {
    let content = '# Breadbrich Engels\n\nYou are Breadbrich Engels.';

    const newName = 'C.L.A.U.D.E';
    content = content.replace(/^# Breadbrich Engels$/m, `# ${newName}`);
    content = content.replace(/You are Breadbrich Engels/g, `You are ${newName}`);
...
describe('CLAUDE.md template copy', () => {
  ...
  function simulateRegister(
    ...
    if (assistantName !== 'Breadbrich Engels') {
      const mdFiles = fs
      ...
      for (const mdFile of mdFiles) {
        ...
        content = content.replace(/^# Breadbrich Engels$/m, `# ${assistantName}`);
  ...
  it('copies global template for non-main group', () => {
    ...
    expect(content).toContain('You are Breadbrich Engels');
  ...
  it('custom name propagates to all channels and groups', () => {
    ...
    for (const folder of [
      ...
      expect(content).not.toContain('Breadbrich Engels');
    }
  });
