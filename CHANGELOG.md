...
- **Auto-deploy now drains in-flight agent containers before triggering `safe-deploy.sh`.** Previously, when a deploy fired while an agent was mid-response, the orchestrator's SIGTERM would interrupt the run — the agent's reply got dropped and `agent_runs.status` was rewritten as `interrupted` on the next boot. The fix: `setup/auto-deploy.sh` now checks `docker ps --filter name=nanoclaw-` before calling `safe-deploy.sh`; if any agent containers are alive, it defers and lets the next 2-min timer tick try again. Two safety valves: (1) a 15-min `MAX_DEFER_SECONDS` cap so a stuck container can't block deploys forever, (2) a sentinel file at `/opt/breadbrich-backups/.deploy-force` that bypasses the drain check on a single tick (for emergency hot-fixes). Manual `safe-deploy.sh` invocations are unaffected — the drain only gates *automatic* triggers. State is tracked in `/run/breadbrich-deploy-deferred-since` and cleared on each successful deploy or "already up to date" tick.
