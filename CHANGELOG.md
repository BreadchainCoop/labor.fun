...
- **kb-ui can now be served behind an authenticating reverse proxy** (the hosted control plane mounts each tenant's dashboard at `https://cloud.lab0r.fun/dashboard/orgs/<orgId>/kb/*`). Two new env vars, both off by default: **`KB_BASE_PATH`** — a path prefix that every URL kb-ui generates now carries (nav/breadcrumb/doc links, plugin nav cards, and the `fetch()` in the inline drag-and-drop script), routed through one `url()` helper; empty (the self-host default) makes `url()` the identity function, and the rendered pages are byte-identical to before (asserted by test against 13 pages). **`KB_PROXY_SECRET`** — a per-tenant shared secret; when a request's `X-KB-Proxy-Secret` matches it (constant-time compare), kb-ui authenticates from `X-KB-User`/`X-KB-Roles` instead of Basic Auth. There is no separate "trust proxy" flag: a wrong/absent secret means the `X-KB-*` headers are ignored **entirely** and Basic Auth applies exactly as before. Roles are default-deny — only `admin`/`superadmin`/`coordinator`/`resident` are recognised (the control plane maps its only role, `owner`, to `admin`; a new CP role must be added to that map or it grants nothing), and asserted roles never fall back to the `KB_*` env lists. Same-origin enforcement on mutations now compares against `X-Forwarded-Host` on a proxy-authenticated request and the real `Host` otherwise, tightened from a substring test to an exact host match that rejects opaque `null`/unparseable origins. The prefix is delivered **only** in the env — kb-ui never reads `X-Forwarded-Prefix`, since a prefix decides where every link on the page points and that header rides on unauthenticated requests too — and a `KB_BASE_PATH` containing a `.`/`..` segment (in any percent-encoded spelling) is refused, because the browser would resolve it back out of the prefix. The same-origin check is a real second opinion rather than a formality: the proxy relays the client's genuine `Origin`/`Referer` instead of synthesising one, so a mutation carrying only a foreign `Referer` is refused by kb-ui. The proxy-asserted `X-KB-User` is HTML-escaped where it is rendered. Plugin dashboard slices get `url` / `BASE_PATH` / `isSameOrigin(req)` in `deps` (docs: `docs/PLUGINS.md` §2c, `docs/HOSTED.md`). `kb-ui/server.mjs` only calls `app.listen()` when it is the process entrypoint, so it is now importable under test.
