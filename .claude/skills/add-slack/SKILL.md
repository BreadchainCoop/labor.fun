...
This skill adds Slack support to your assistant, then walks through interactive setup.
...
```bash
git remote add slack https://github.com/qwibitai/salem-slack.git
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.<your-service-label>
```
...
>
> The JID format is `slack:<channel-id>` — e.g. `slack:C0123456789` (replace `C0123456789` with your channel's actual ID).
...
```bash
tail -f logs/<your-service>.log   # log filename is profile-specific
```
...
4. Service is running: `launchctl list | grep <your-service-label>`
...
6. Restart: `launchctl kickstart -k gui/$(id -u)/com.<your-service-label>`
