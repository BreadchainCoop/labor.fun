...
This skill switches Breadbrich Engels's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
```bash
grep -q 'breadbrich proxy' /etc/pf.conf 2>/dev/null || echo '# breadbrich proxy — block LAN access to credential proxy
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo breadbrich-agent:latest "Container OK"
```
