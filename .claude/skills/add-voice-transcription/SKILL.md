...
description: Add voice message transcription to Breadbrich Engels using OpenAI's Whisper API. Automatically transcribes WhatsApp voice notes so the agent can read and respond to them.
---
...
```bash
tail -f logs/breadbrich.log | grep -i voice
```
