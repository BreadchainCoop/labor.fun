...
description: Add a macOS menu bar status indicator for Breadbrich Engels. Shows a bolt icon with a green/red dot indicating whether Breadbrich Engels is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows Breadbrich Engels's running status and lets the user
...
```bash
launchctl list | grep com.breadbrich.statusbar
```
...
Create `~/Library/LaunchAgents/com.breadbrich.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.breadbrich.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.breadbrich.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see Breadbrich Engels's status and control the service.
>
> - **Green dot** — Breadbrich Engels is running
> - **Red dot** — Breadbrich Engels is stopped
>
...
```bash
launchctl unload ~/Library/LaunchAgents/com.breadbrich.statusbar.plist
rm ~/Library/LaunchAgents/com.breadbrich.statusbar.plist
rm dist/statusbar
```
