...
Adds Parallel AI MCP integration to Breadbrich Engels for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Breadbrich Engels scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Breadbrich Engels already set up and running
...
Add Parallel AI MCP servers after the breadbrich server:
```typescript
const mcpServers: Record<string, any> = {
  breadbrich: ipcMcp
};
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo breadbrich-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep breadbrich  # macOS
...
- Check task runs: `tail -f logs/breadbrich.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.breadbrich` (macOS) or `systemctl --user restart breadbrich` (Linux)
