...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.breadbrich.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/breadbrich.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/breadbrich.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.breadbrich.plist && launchctl load ~/Library/LaunchAgents/com.breadbrich.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart breadbrich` (Linux)
