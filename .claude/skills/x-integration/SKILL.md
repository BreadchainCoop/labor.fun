---
name: x-integration
description: X (Twitter) integration for Breadbrich Engels. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** Breadbrich Engels v1.0.0. Directory structure may change in future versions.
...
1. **Breadbrich Engels is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep breadbrich (macOS) or systemctl --user status breadbrich (Linux)
```
...
| `logs/breadbrich.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into Breadbrich Engels, make the following modifications:
...
**Verify success:**
```bash
launchctl list | grep breadbrich  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status breadbrich
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/breadbrich.log | tail -20

# Script errors
grep -i "error\|failed" logs/breadbrich.log | tail -20
```
...
docker run breadbrich-agent ls -la /app/src/skills/
```
