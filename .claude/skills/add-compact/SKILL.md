...
> **Note:** `upstream` is the remote pointing to `qwibitai/salem`. If using a different remote name, substitute accordingly.
...
1. Start Breadbrich Engels in dev mode: `npm run dev`
...
```bash
git clone <your-fork> /tmp/breadbrich-test
cd /tmp/breadbrich-test
...
- **No config file.** Breadbrich Engels's philosophy is customization through code changes, not configuration sprawl.
