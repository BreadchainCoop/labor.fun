...
**`qwibitai/salem`** (upstream) — core engine with skill definitions (`.claude/skills/`). No channel code on `main`.

**Channel forks** (`breadbrich-whatsapp`, `breadbrich-telegram`, `breadbrich-slack`, etc.) — each fork = upstream + one channel's code applied. Users clone upstream, then merge a fork into their clone to add a channel.
...
user clones upstream main
  ├── merges breadbrich-whatsapp fork  → adds WhatsApp
...
```bash
# In your local breadbrich checkout
git checkout main && git pull

# For a fork:
git fetch breadbrich-whatsapp
git checkout -B whatsapp-merge breadbrich-whatsapp/main
git merge main
...
git push breadbrich-whatsapp HEAD:main
...
1. Fork `breadbrich` to `breadbrich-{channel}`
