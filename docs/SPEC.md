# Breadbrich Engels Specification
...
│   ├── breadbrich.log               # Host stdout
│   └── breadbrich.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.breadbrich.plist         # macOS service configuration
```
...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Breadbrich Engels';
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'breadbrich-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your breadbrich installation
...
Breadbrich Engels uses a hierarchical memory system based on CLAUDE.md files.
...
7. Router invokes Claude Agent SDK:
   ...
   └── mcpServers: breadbrich (scheduler)
   │
   ▼
...
Messages must start with the trigger pattern (default: `@Breadbrich Engels`):
- `@Breadbrich Engels what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @Breadbrich Engels` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Breadbrich Engels what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Breadbrich Engels what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Breadbrich Engels add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Breadbrich Engels remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Breadbrich Engels list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Breadbrich Engels remember I prefer dark mode` | Add to global memory |

---

## Scheduled Tasks

Breadbrich Engels has a built-in scheduler that runs tasks as full agents in their group's context.
...
```
User: @Breadbrich Engels remind me every Monday at 9am to review the weekly metrics
...
```
User: @Breadbrich Engels at 5pm today, send me a summary of today's emails
...
From any group:
- `@Breadbrich Engels list my scheduled tasks` - View tasks for this group
- `@Breadbrich Engels pause task [id]` - Pause a task
- `@Breadbrich Engels resume task [id]` - Resume a paused task
- `@Breadbrich Engels cancel task [id]` - Delete a task

From main channel:
- `@Breadbrich Engels list all tasks` - View tasks from all groups
- `@Breadbrich Engels schedule task for "Family Chat": [prompt]` - Schedule for another group

---

## MCP Servers

### Breadbrich Engels MCP (built-in)

The `breadbrich` MCP server is created dynamically per agent call with the current group's context.
...
Breadbrich Engels runs as a single macOS launchd service.

### Startup Sequence

When Breadbrich Engels starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Breadbrich Engels containers from previous runs
...
### Service: com.breadbrich

**launchd/com.breadbrich.plist:**
```xml
...
<dict>
    ...
    <dict>
        ...
        <string>Breadbrich Engels</string>
    </dict>
    <key>StandardOutPath</key>
    <string>{{PROJECT_ROOT}}/logs/breadbrich.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/breadbrich.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.breadbrich.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.breadbrich.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.breadbrich.plist

# Check status
launchctl list | grep breadbrich
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep breadbrich` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Breadbrich Engels auto-starts container runtime but may fail |
| "Claude Code process exited with code 1" | Session mount path wrong | Ensure mount is to `/home/node/.claude/` not `/root/.claude/` |
| Session not continuing | Session ID not saved | Check SQLite: `sqlite3 store/messages.db "SELECT * FROM sessions"` |
| Session not continuing | Mount path mismatch | Container user is `node` with HOME=/home/node; sessions must be at `/home/node/.claude/` |
| "QR code expired" | WhatsApp session expired | Delete store/auth/ and restart |
| "No groups registered" | Haven't added groups | Use `@Breadbrich Engels add group "Name"` in main |

### Log Location

- `logs/breadbrich.log` - stdout
- `logs/breadbrich.error.log` - stderr
