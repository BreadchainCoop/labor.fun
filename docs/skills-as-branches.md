...
This document covers **feature skills** — skills that add capabilities via git branch merges. This is the most complex skill type and the primary way Breadbrich Engels is extended.

Breadbrich Engels has four types of skills overall. See [CONTRIBUTING.md](../CONTRIBUTING.md) for the full taxonomy:
...
This replaces the previous `skills-engine/` system (three-way file merging, `.breadbrich/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`qwibitai/salem`) maintains:

- `main` — core Breadbrich Engels (no skill code)
...
- `/setup`, `/debug`, `/update-breadbrich`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`qwibitai/salem-skills`)
...
claude plugin install breadbrich-skills@breadbrich-skills --scope project
```
...
Breadbrich Engels's `.claude/settings.json` registers the official marketplace:
...
```
qwibitai/salem-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    breadbrich-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `breadbrich-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-breadbrich` — after merging main, optionally check for skill updates
...
1. Fork `qwibitai/salem` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
   git clone https://github.com/<you>/breadbrich.git
   cd breadbrich
   ```
3. Run Claude Code:
   ```bash
...
Users who previously ran `git clone https://github.com/qwibitai/salem.git` and have local customizations:

1. Fork `qwibitai/salem` on GitHub
2. Reroute remotes:
   ```bash
   git remote rename origin upstream
   git remote add origin https://github.com/<you>/breadbrich.git
...
3. From this point, `origin` = their fork, `upstream` = qwibitai/salem
...
In both cases:
- Delete the `.breadbrich/` directory (no longer needed)
...
This is the same as the existing `/update-breadbrich` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-breadbrich` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to qwibitai/salem:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`qwibitai/salem-skills`)
...
If the community contributor is trusted, they can open a PR to add their marketplace to Breadbrich Engels's `.claude/settings.json`:
...
Once merged, all Breadbrich Engels users automatically discover the community marketplace alongside the official one.
...
A flavor is a curated fork of Breadbrich Engels — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "Breadbrich Engels for Sales," "Breadbrich Engels Minimal," "Breadbrich Engels for Developers").
...
- `upstream` — `qwibitai/salem` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `breadbrich-flavor` for searchability
...
- `.breadbrich/` state directory

Operational skills (`setup`, `debug`, `update-breadbrich`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
Before:
```bash
git clone https://github.com/qwibitai/Breadbrich Engels.git
cd Breadbrich Engels
claude
```
...
```
1. Fork qwibitai/salem on GitHub
2. git clone https://github.com/<you>/breadbrich.git
3. cd breadbrich
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/qwibitai/salem.git`
- Check if `origin` points to the user's fork (not qwibitai). If it points to qwibitai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install breadbrich-skills@breadbrich-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-breadbrich, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`qwibitai/salem-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
### Update skill (`/update-breadbrich`)
...
>
> We've simplified how skills work in Breadbrich Engels. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.breadbrich/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.breadbrich/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to Breadbrich Engels's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
