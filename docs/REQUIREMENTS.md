# Breadbrich Engels Requirements
...
Breadbrich Engels gives you the core functionality without that mess.
...
> **Note:** Telegram, Slack, Discord, Gmail, and Apple Container skills already exist. See the [skills documentation](https://docs.salem.dev/integrations/skills-system) for the full list.
...
- Trigger: `@Breadbrich Engels` prefix (case insensitive), configurable via `ASSISTANT_NAME` env var
...
- Custom `breadbrich` MCP server (inside container) provides scheduling tools
...
- `/update-breadbrich` - Pull upstream changes, merge with customizations
...
- **Trigger**: `@Breadbrich Engels` (case insensitive)
- **Response prefix**: `Breadbrich Engels:`
...
**Breadbrich Engels** - A reference to Clawdbot (now OpenClaw).
